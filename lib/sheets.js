 ...
 *
 * Кожна категорія тепер має поле `parent` — id батьківської категорії
 * (порожньо для категорій верхнього рівня). Колонку "parent" потрібно
 * додати в лист Categories вручну — якщо її ще немає, всі категорії
 * просто лишаються верхнього рівня, як і раніше.
 */
export async function getAllCategories() {
  const { categories } = await fetchAllSheets();
  const map = buildCategoryMap(categories);
