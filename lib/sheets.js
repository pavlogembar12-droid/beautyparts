...
export async function searchProducts(filters = {}) {
  ...
  // Кілька категорій одночасно (з мультивибору в каталозі) — через кому.
  // Якщо серед обраних є категорія верхнього рівня — товари її підкатегорій теж підходять.
  let categorySlugs = null;
  const selectedCats = (category || '').split(',').map((s) => s.trim()).filter(Boolean);
  if (selectedCats.length > 0) {
    categorySlugs = new Set(selectedCats);
    for (const c of categories) {
      if (c.parent && selectedCats.includes(c.parent)) categorySlugs.add(c.slug);
    }
  }
