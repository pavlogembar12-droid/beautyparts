...
async function fetchSheetTab(sheetName) {
  ...
  console.log(`[DEBUG fetchSheetTab] requesting "${sheetName}":`, url);
  try {
    ...
    console.log(`[DEBUG fetchSheetTab] "${sheetName}" response preview:`, text.slice(0, 200));
...
export async function getAllCategories() {
  ...
  // ТИМЧАСОВЕ ЛОГУВАННЯ для діагностики — видалити після підтвердження,
  // що категорії відображаються коректно. Видно у Vercel → Logs.
  console.log('[DEBUG categories] count:', categories.length);
  console.log('[DEBUG categories] first row:', JSON.stringify(categories[0]));
