...
async function fetchAllSheets() {
  try {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 8000); // 8с — щоб не вішати всю збірку, якщо Sheets повільні
    let res;
    try {
...
function buildCategoryMap(categories) {
  ...
  for (const c of categories) {
    ...
    map.set(id, {
      ...
      icon: c.icon || c.Icon || '',
      // "parent" — id батьківської категорії з листа Categories.
      // Порожньо/відсутнє = це категорія верхнього рівня.
...
// ⚠️ Ключова функція — від неї залежить увесь сайт (каталог, товари,
// бренди, sitemap). Раніше випадково загубилась при редагуванні — якщо
// колись знову побачиш помилку "getAllProducts is not defined/exported"
// в логах Vercel, значить хтось знову видалив саме цю функцію.
...
export async function getProductsByCategory(categorySlug) {
  ...
  const categoryMap = buildCategoryMap(categories);
  // Якщо це категорія верхнього рівня — показуємо і її товари, і товари всіх підкатегорій.
...
/**
 * Категорії, згруповані в дерево: кожна категорія верхнього рівня
 * (без parent) отримує масив `children` зі своїми підкатегоріями.
 * Зручно для фільтра в каталозі та для сторінки категорії.
 */
...
export async function getAllBrands() {
  ...
  const map = new Map();
  for (const p of products) {
    // "Універсальний" — це не реальний бренд, а помилкове значення поля brand
    // у товарах, що підходять до всіх машинок. Ховаємо його зі списку брендів,
    // самі товари лишаються доступні через категорії/каталог.
...
export async function searchProducts(filters = {}) {
  ...
  // Якщо обрано категорію верхнього рівня — товари її підкатегорій теж підходять.
