...
const SHEET_ID = '1AbKN0l43knJmUH_0jBvmKz1P1-HUwjz5JDyZ-_IsKOQ';
const CATEGORIES_GID = '1684975903';

/**
 * Читає аркуш Categories напряму з Google Таблиці через публічний
 * CSV-експорт по gid — в обхід Apps Script doGet, який стабільно
 * повертав categories:[] через конфлікт двох однойменних аркушів
 * або інший внутрішній баг Apps Script.
 * Products і Models читаються як і раніше через Apps Script doGet.
 */
async function fetchCategoriesCSV() {
  try {
    const url = `https://docs.google.com/spreadsheets/d/${SHEET_ID}/export?format=csv&gid=${CATEGORIES_GID}`;
    ...
    if (!res.ok) throw new Error(`CSV export відповів ${res.status}`);
    const text = await res.text();
    return parseCsv(text);
  } catch (err) {
    console.error('Помилка завантаження категорій через CSV:', err);
    return [];
  }
}

/**
 * Мінімальний CSV-парсер з підтримкою лапок і BOM.
 */
...
async function fetchAllSheets() {
  try {
    // Паралельно запускаємо обидва запити щоб скоротити час завантаження
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 8000);

    let appsScriptRes, categoriesRaw;
    try {
      [appsScriptRes, categoriesRaw] = await Promise.all([
        fetch(SHEET_API_URL, {
          next: { revalidate: REVALIDATE_SECONDS },
          redirect: 'follow',
          signal: controller.signal,
    ...
    if (!appsScriptRes.ok) throw new Error(`Sheet API відповів ${appsScriptRes.status}`);
    const data = await appsScriptRes.json();

    const products = data.products || data.Products || (Array.isArray(data) ? data : []) || [];
    const models = data.models || data.Models || [];
    const categories = csvRowsToObjects(categoriesRaw);
...
function buildCategoryMap(categories) {
  ...
  for (const c of categories) {
    ...
    map.set(id, {
      ...
      seoTitle: c.seoTitle || c.SeoTitle || '',
      seoDescription: c.seoDescription || c.SeoDescription || '',
    });
  }
  return map;
}
...
/**
 * fetch з жорстким таймаутом для запитів ЗАПИСУ (save/delete) до Apps
 * Script. Без цього, якщо Apps Script Web App "зависає" (не відповідає
 * взагалі), fetch() ніколи не завершується — кнопка "Видалити"/"Зберегти"
 * в адмінці лишається в стані завантаження назавжди, без жодної помилки.
 */
async function fetchWithTimeout(url, options, timeoutMs = 15000) {
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      throw new Error('Google Sheets не відповідає (тайм-аут). Спробуйте ще раз за хвилину.');
    }
    throw err;
  } finally {
    clearTimeout(timeoutId);
