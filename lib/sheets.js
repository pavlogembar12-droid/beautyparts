...
/**
 * CSV-парсер (state machine): підтримує коми всередині лапок,
 * екрановані подвійні лапки ("") та переноси рядків усередині поля.
 * Google CSV-експорт обгортає КОЖНЕ поле в лапки, включно з
 * заголовками — тому критично, щоб самі лапки-роздільники ніколи не
 * потрапляли в результат (тільки екрановані "" всередині значення).
 */
function parseCsv(text) {
  // Прибираємо BOM, якщо є — інакше перший заголовок буде "\uFEFFid"
  // замість "id", і жодне поле ніколи не знайде значення в obj[key].
...
function rowsToObjects(rows) {
  ...
  // trim() на заголовках критичний: зайвий пробіл у "id " робить
  // obj["id "] замість obj["id"], і весь подальший код (c.id, m.id...)
  // мовчки не знаходить значення.
  const headers = rows[0].map((h) => h.trim());
  const out = [];
  for (let i = 1; i < rows.length; i++) {
    const r = rows[i];
    if (!r[0] || !r[0].trim()) continue; // порожній id — пропускаємо рядок
    const obj = {};
    for (let j = 0; j < headers.length; j++) obj[headers[j]] = r[j] !== undefined ? r[j].trim() : '';
    out.push(obj);
  }
  return out;
}
 ...
 *
 * Якщо відповідь виявляється не чистим CSV (напр. gviz повернув
 * JS-обгортку google.visualization.Query.setResponse(...) замість
 * сирого CSV), не згодовуємо це в парсер — повертаємо порожньо і
 * логуємо помилку, щоб не показати сторінку зі "сміттєвими" даними.
 */
async function fetchSheetTab(sheetName) {
  ...
  try {
    ...
    if (text.trim().startsWith('google.visualization') || text.trim().startsWith('<')) {
      console.error(`Аркуш "${sheetName}": відповідь не є CSV (перші 80 символів): ${text.slice(0, 80)}`);
      return [];
    }
...
async function fetchAllSheets() {
  // Послідовно, не Promise.all — паралельні запити з майже ідентичним
  // URL (відрізняється лише sheet=) провокували Next.js/Google повертати
  // одну й ту саму закешовану відповідь на всі три запити одночасно.
  const products = await fetchSheetTab('Products');
  const categories = await fetchSheetTab('Categories');
  const models = await fetchSheetTab('Models');
  return { products, categories, models };
}

// cache() дедуплікує виклики fetchAllSheets в межах одного рендеру
// сторінки: навіть якщо getAllProducts(), getCategoryTree(),
// getAllBrands() і getAllModels() усі викликаються на одній сторінці
// (як на головній), реальні HTTP-запити до Google підуть лише один
// раз, а не 4 паралельні набори — саме паралельні набори й були
// причиною плутанини відповідей між аркушами.
const getCachedSheets = cache(fetchAllSheets);
...
export async function getAllProducts() {
  const { products, categories, models } = await getCachedSheets();
...
export async function getAllCategories() {
  const { categories, products } = await getCachedSheets();
...
export async function getProductsByCategory(categorySlug) {
  const { categories } = await getCachedSheets();
  const categoryMap = buildCategoryMap(categories);
...
export async function getAllModels() {
  const { models } = await getCachedSheets();
 ...
 * =====================================================================
 * Пише в лист Categories тим самим механізмом upsertRow/deleteRowById,
 * що й товари. id категорії — це її slug (напр. "blades") і одночасно
 * первинний ключ рядка в таблиці, тому при створенні нової категорії
 * id генерується зі slugify(label), якщо адмін не вписав його вручну.
 */
