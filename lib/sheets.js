...
async function fetchAllSheets() {
  try {
    ...
    let res;
    try {
      // Кешування тепер контролюється на рівні СТОРІНКИ (export const
      // revalidate / dynamic), а не тут. Раніше next:{revalidate:300}
      // тут означало, що навіть адмінські сторінки з force-dynamic
      // все одно бачили дані, застарілі до 5 хвилин — router.refresh()
      // після видалення/збереження не допомагав, бо кеш був саме на
      // рівні цього fetch, а не рендеру сторінки.
